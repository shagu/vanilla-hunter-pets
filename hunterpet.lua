...
do -- database
  ...
  local query = mysql:execute([[
    SELECT entry, patch, name, base_attack_time, rank, display_id1, pet_family, level_min, level_max, spell_list_id, pet_spell_list_id, unit_class FROM `creature_template` WHERE ( static_flags1 & 16) AND pet_family > 0 ORDER BY pet_family, level_min, level_max, name
  ]])
  ...
  while query:fetch(creature_template, "a") do
    ...
    pets[tableid].family = creature_template.pet_family
